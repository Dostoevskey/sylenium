...
public class LocalisationFileReader implements ConfigObserver {
    ...
    public String getLanguageValue(final String key) {
        return Optional.ofNullable(languageResources.get(key)).orElseThrow(() -> new NoSuchLocalisedPropertyException("No such language property exists: " + key));
    }

    private void readLanguageResourcesFromFile() {
        final Properties temp = new Properties();
        languageResources.putAll(safelyFindFile(temp) .entrySet()
                .stream()
                .collect(
                        Collectors.toMap(
    ...
    private Properties safelyFindFile(final Properties props) {
        final String noSuchValue = String.format("Language file %s does not exist in test/resources/localisation/", config.localisationFile());
        InputStream is = null;
        try {
