...
public class Sylenium {
  ...
  private static final SyleniumConfig config = new SyleniumConfig();
  ...
  public String localised(final String key) {
      return localisationResources.get(key);
  }
  ...
  private Sylenium buildRuntimeLanguageProperties() {
    ...
    try {
      is =
          Optional.ofNullable(getClass().getClassLoader().getResourceAsStream(config.language()))
