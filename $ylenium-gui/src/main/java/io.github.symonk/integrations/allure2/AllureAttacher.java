...
public class AllureAttacher {
  ...
  public <T extends Modelable> void attachModelUsingCustomName(final T modelObject, final String name) {
    Allure.addAttachment(name, JSON, gson.toJson(modelObject));
  }

  public <T extends Modelable> void attachModelUsingDefaultName(final T modelObject) {
    Allure.addAttachment(prepareAttachment(modelObject), JSON, gson.toJson(modelObject));
  }

  private <T extends Modelable> String prepareAttachment(final T modelObject) {
